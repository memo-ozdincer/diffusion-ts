#!/usr/bin/env python
"""Parallel TS search v4: further improvements for 0.5A noise.

Changes from v3:
  - GAD with energy guard: abort if energy rises > 20 eV from handoff
  - Better nneg3 candidate handling: also perturb along eigenvectors
  - Add re-minimization rescue: when stuck at high morse, re-minimize and retry
  - Early GAD abort on divergence (force > 5.0 sustained) saves budget for better candidates
  - Smarter step budget allocation: don't burn full budget on bad candidates
"""
...
def run_gad_guarded(predict_fn, coords0, atomic_nums, atomsymbols,
                     GADConfig, vib_eig, gad_steps=5000, energy_guard=20.0):
    """Run GAD with energy guard and early abort on divergence.

    Returns (result, actually_used_steps).
    If energy rises more than energy_guard from start, abort early.
    If force stays > 5.0 for 200+ consecutive steps, abort early.
    """
    from src.core_algos.saddle_optimizer import run_gad_saddle_search

    # Get starting energy
    out0 = predict_fn(coords0.reshape(-1, 3), atomic_nums, do_hessian=True)
    e_start = float(out0["energy"].item()) if hasattr(out0["energy"], 'item') else float(out0["energy"])

    # Run GAD in chunks to check energy guard
    chunk_size = 500
    current_coords = coords0
    total_steps = 0
    high_force_count = 0

    for chunk_start in range(0, gad_steps, chunk_size):
        remaining = gad_steps - chunk_start
        ...
        result = run_gad_saddle_search(
            predict_fn, current_coords, atomic_nums, atomsymbols, gad_cfg)
        ...
        # Energy guard
        e_now = result["final_energy"]
        if e_now > e_start + energy_guard:
            result["total_steps"] = total_steps
            result["abort_reason"] = "energy_guard"
            return result, total_steps

        # Force divergence guard
        f_now = result.get("final_force_norm", 0)
        if f_now > 5.0:
            high_force_count += 1
            if high_force_count >= 3:  # 3 chunks = 1500 steps with high force
                result["total_steps"] = total_steps
                result["abort_reason"] = "force_divergence"
                return result, total_steps
        else:
            high_force_count = 0

        current_coords = result["final_coords"]
...
def run_gad_with_restarts(predict_fn, handoff_coords, atomic_nums, atomsymbols,
    ...
    """Run GAD with restarts, energy guards, and re-minimization rescue."""
    steps_used = 0
    best_result = None

    # Phase 1: Coarse exploration with energy guard
    coarse_steps = min(5000, max_total_steps)
    result, steps = run_gad_guarded(
        predict_fn, handoff_coords, atomic_nums, atomsymbols,
        GADConfig, vib_eig, gad_steps=coarse_steps)
    steps_used += steps
    ...
    # Phase 2: If stuck at n_neg=2 with low force, perturb along eigenvectors
    if remaining > 500 and result["final_morse_index"] == 2 and result.get("final_force_norm", 999) < 0.5:
        ...
        if evecs.shape[1] >= 2:
            # Try both eigenvectors (1st and 2nd)
            for evec_idx in [1, 0]:
                v = evecs[:, evec_idx].reshape(-1, 3).to(final_coords.dtype)
                for scale in [0.15, -0.15, 0.3, -0.3]:
                    if remaining <= 500:
                        break
                    perturbed = final_coords + scale * v
                    if _min_dist(perturbed) < 0.3:
                        continue

                    retry_steps = min(2000, remaining)
                    retry_result, retry_s = run_gad_guarded(
                        predict_fn, perturbed, atomic_nums, atomsymbols,
                        GADConfig, vib_eig, gad_steps=retry_steps)
                    steps_used += retry_s
                    remaining = max_total_steps - steps_used
                    ...
                    if best_result is None or (
                        ...
                        retry_result.get("final_force_norm", 999) < (best_result.get("final_force_norm", 999))
                    ):
                        best_result = retry_result

    # Phase 3: If n_neg=1 but force not converged, use fine dt
    check = best_result if (best_result and best_result["final_morse_index"] == 1) else result
    remaining = max_total_steps - steps_used
    if remaining > 500 and check["final_morse_index"] == 1 and check.get("final_force_norm", 999) < 1.0:
        from src.core_algos.saddle_optimizer import run_gad_saddle_search
        fine_cfg = GADConfig(
        ...
        fine_result = run_gad_saddle_search(
            predict_fn, check["final_coords"], atomic_nums, atomsymbols, fine_cfg)
        steps_used += fine_result["total_steps"]

        if fine_result["converged"]:
            return fine_result, steps_used
        if fine_result.get("final_force_norm", 999) < check.get("final_force_norm", 999):
            if best_result is None or fine_result.get("final_force_norm", 999) < best_result.get("final_force_norm", 999):
                best_result = fine_result

    # Phase 4: Re-minimization rescue
    # If GAD produced a geometry with high force/high morse, re-minimize and try GAD again
    remaining = max_total_steps - steps_used
    rescue_candidate = best_result if best_result else result
    if (remaining > 1000 and rescue_candidate["final_morse_index"] > 2 and
        rescue_candidate.get("final_force_norm", 999) > 0.5):
        # Re-minimize from current position
        nr_rescue_cfg = NRConfig(n_steps=500, force_converged=0.01, min_interatomic_dist=0.3)
        nr_rescue = run_nr_minimization(
            predict_fn, rescue_candidate["final_coords"], atomic_nums, atomsymbols, nr_rescue_cfg)
        steps_used += nr_rescue["total_steps"]

        if nr_rescue["converged"]:
            # Now try GAD from the re-minimized geometry with perturbation
            min_coords = nr_rescue["final_coords"]
            ...
            for scale in [0.3, -0.3, 0.5]:
                ...
                rescue_gad_result, rescue_gad_s = run_gad_guarded(
                    predict_fn, perturbed, atomic_nums, atomsymbols,
                    GADConfig, vib_eig, gad_steps=min(2000, remaining))
                steps_used += rescue_gad_s
                remaining = max_total_steps - steps_used

                if rescue_gad_result["converged"]:
                    return rescue_gad_result, steps_used
                if rescue_gad_result["final_morse_index"] == 1 and rescue_gad_result.get("final_force_norm", 999) < (best_result.get("final_force_norm", 999) if best_result else 999):
                    best_result = rescue_gad_result

    final = best_result if best_result else result
    return final, steps_used


def worker(seed, noise_std=0.3):
    """Single TS search with v4 strategy."""
    ...
    nneg4plus_points = [rec for rec in traj if rec["n_neg"] >= 4 and "coords" in rec]
    ...
    if nneg3_points:
        ...
        if len(nneg3_points) > 1:
            candidates.append(("first_nneg3", nneg3_points[0]))

    # For very high noise: also try highest-index early points
    if nneg4plus_points and not nneg1_points and not nneg2_points:
        # Only nneg3+ available -- try the last one with a lower displacement
        candidates.append(("last_nneg4plus", nneg4plus_points[-1]))
    ...
    # If NR converged but didn't give us nneg1 candidates, also try
    # perturbing the minimum along the 2nd and 3rd eigenvectors
    if nr_result["converged"] and not nneg1_points and nneg0_points:
        min_coords = nr_result["final_coords"]
        out_min = predict_fn(min_coords, ATOMIC_NUMS, do_hessian=True)
        evals_min, evecs_min, _ = vib_eig(out_min["hessian"], min_coords, ATOMSYMBOLS)
        for evec_idx in [1, 2]:
            if evecs_min.shape[1] > evec_idx:
                v = evecs_min[:, evec_idx].reshape(-1, 3).to(min_coords.dtype)
                for scale in [0.4, -0.4]:
                    perturbed = min_coords + scale * v
                    if _min_dist(perturbed) >= 0.3:
                        fake_rec = {"step": -1, "n_neg": 0, "coords": perturbed}
                        candidates.append((f"min_evec{evec_idx}_{scale}", fake_rec))

    if not candidates:
        # Last resort: use the final NR coords regardless of n_neg
        if traj:
            candidates.append(("last_nr_point", traj[-1]))
        else:
            return {
...
def main():
    ...
    print(f"Parallel TS search v4: {args.n_seeds} seeds, {args.n_workers} workers, "
    ...
    out_path = os.path.join(out_dir, "parallel_results_v4.json")
