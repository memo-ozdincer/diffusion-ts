#!/usr/bin/env python
"""Parallel TS search v5: multi-start NR + targeted escape from wrong saddles.

Key insight: at 0.5A noise, many seeds converge to n_neg=2 saddles.
New strategies:
  1. Multi-start NR: try full noise AND 60% noise for more diverse handoff points
  2. When stuck at n_neg=2 (converged force < 0.1), run NR to reach nearby minimum,
     then perturb along eigenvectors and retry GAD ("scoop and retry")
  3. Energy guard to prevent wasting steps on divergent trajectories
  4. Separate coarse and fine GAD phases
"""
...
def collect_candidates_from_nr(nr_result, predict_fn, vib_eig, label_prefix=""):
    """Extract handoff candidates from an NR trajectory."""
    ...
    if nneg1_points:
        ...
        if len(nneg1_points) > 2:
            ...
            candidates.append((f"{pfx}mid_nneg1", nneg1_points[mid]))
        if len(nneg1_points) > 4:
            q1 = len(nneg1_points) // 4
            q3 = 3 * len(nneg1_points) // 4
            candidates.append((f"{pfx}q1_nneg1", nneg1_points[q1]))
            candidates.append((f"{pfx}q3_nneg1", nneg1_points[q3]))
    ...
    # Minimum perturbation candidates
...
def scoop_and_retry(predict_fn, stuck_coords, atomic_nums, atomsymbols,
                     ...
                     run_gad_saddle_search, max_steps=5000):
    """When stuck at n_neg=2, minimize from there, then try GAD from the minimum."""
    # Phase 1: Re-minimize to find nearby minimum
    ...
    # Phase 2: Perturb minimum along eigenvectors and try GAD
    min_coords = nr_result["final_coords"]
    out_min = predict_fn(min_coords, atomic_nums, do_hessian=True)
    evals_min, evecs_min, _ = vib_eig(out_min["hessian"], min_coords, atomsymbols)
...
def worker(seed, noise_std=0.3):
    """Single TS search with v5 multi-start strategy."""
    ...
    nr_result1 = run_nr_minimization(predict_fn, noisy_coords, ATOMIC_NUMS, ATOMSYMBOLS, nr_cfg)
    total_nr_steps += nr_result1["total_steps"]
    cands1 = collect_candidates_from_nr(nr_result1, predict_fn, vib_eig, "A_")
    all_candidates.extend(cands1)

    # Start 2: Reduced noise (60% of full noise, only for high noise)
    if noise_std >= 0.4 and len([c for c in cands1 if "nneg1" in c[0]]) < 3:
        # Scale noise down to 60% of original
        reduced_noise = ISOPROPANOL_COORDS + (noisy_coords - ISOPROPANOL_COORDS) * 0.6
        if _min_dist(reduced_noise) >= 0.3:
            nr_result2 = run_nr_minimization(predict_fn, reduced_noise, ATOMIC_NUMS, ATOMSYMBOLS, nr_cfg)
            total_nr_steps += nr_result2["total_steps"]
            cands2 = collect_candidates_from_nr(nr_result2, predict_fn, vib_eig, "B_")
    ...
    # ========== Phase 2: GAD with multi-phase approach ==========
    # Sort candidates: nneg1 first, then nneg2, then perturbed minima
    def candidate_priority(item):
        ...
        if "nneg1" in label:
            ...
            return 10
        elif "min_ev0" in label:
            return 20
        elif "min_ev" in label:
            return 21
        elif "nneg3" in label:
            return 30
        else:
    ...
    for label, rec in all_candidates:
        ...
        # Scoop-and-retry if stuck at n_neg=2 with low force
        if (gad_result["final_morse_index"] == 2 and
            gad_result.get("final_force_norm", 999) < 0.1):
            ...
            if scoop_result and scoop_result["converged"]:
                ...
                best_label = f"{label}_scooped"
                best_step = rec["step"]
                best_nneg = rec["n_neg"]
                break

        # Track best non-converged
...
def main():
    ...
    print(f"Parallel TS search v5: {args.n_seeds} seeds, {args.n_workers} workers, "
    ...
    out_path = os.path.join(out_dir, "parallel_results_v5.json")
